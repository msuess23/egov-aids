...
def _build_supervised_preprocessor(categorical_cols: list, numeric_cols: list):
    """
    Constructs the dynamic preprocessing pipeline.
    Imputation and transformation are applied just-in-time within the training loop.
    Log1p transformation is included to handle heavy-tailed financial and count data 
    for linear models (LogReg), while remaining neutral for tree-based models.
    """
    transformers = []
    if numeric_cols:
...
def tune_supervised_model(X_train: pd.DataFrame, y_train: pd.Series, groups_train: pd.Series, 
    ...
    """
    preprocessor = _build_supervised_preprocessor(categorical_cols, numeric_cols)
